...
onMounted(() => {
    menuToggle.value = document.getElementById('menu-toggle');
    mobileMenu.value = document.getElementById('mobile-menu');
    menuToggle.value.addEventListener('click', () => {
        mobileMenu.value.classList.toggle('hidden');
        mobileMenu.value.classList.toggle('translate-y-0');
        mobileMenu.value.classList.toggle('opacity-100');
    });
});
</script>
...
<template>
    <header class="bg-white text-black border-b-2 border-primary shadow-md fixed top-0 left-0 w-full z-10">
        ...
        <!-- Navigation Mobile -->
        <nav id="mobile-menu"
            class="hidden md:hidden flex-col bg-white border-b-2 border-t-2 border-t-gray-200 border-primary text-black p-6 space-y-4 absolute inset-x-0 top-[4.5rem] shadow-lg rounded-b-lg ">
